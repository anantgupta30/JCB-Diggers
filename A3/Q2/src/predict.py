...
def predict_C(
    ...
    if hasattr(model, "score_edges") and getattr(model, "expects_edge_index", False):
        pos_scores = model.score_edges(dataset.x, dataset.edge_index, pos)  # [P]
        neg_scores = model.score_edges(dataset.x, dataset.edge_index, neg.view(P * K, 2)).view(P, K)
    elif hasattr(model, "score_edges"):
        pos_scores = model.score_edges(dataset.x, pos)                  # [P]
        neg_scores = model.score_edges(dataset.x, neg.view(P * K, 2)).view(P, K)
    else:
